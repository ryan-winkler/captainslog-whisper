...
func GenerateOrLoad(certDir string, hostnames []string, logger *slog.Logger) (*tls.Config, error) {
	certFile := filepath.Join(certDir, "voxscribe.crt")
	keyFile := filepath.Join(certDir, "voxscribe.key")
	...
	template := &x509.Certificate{
		...
		Subject: pkix.Name{
			Organization: []string{"VoxScribe (self-signed)"},
			CommonName:   "VoxScribe Local",
		},
		NotBefore:             time.Now(),
